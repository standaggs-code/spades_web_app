...
window.exportGame = () => {
    ...
    let txt = `♠️ SPADES: ${winnerTeam.members.join(' & ')} WIN! ♠️\nScore: ${state.teams[0].score}-${state.teams[1].score}\n\n`;
    txt += state.history.map((h, i) => `R${i+1}: T1(${h.t1.bid}/${h.t1.teamGot}) T2(${h.t2.bid}/${h.t2.teamGot})`).join('\n');
